...
DEFAULT_CSS_FILENAME = os.path.join(os.path.dirname(__file__), 'default.css')
...
def generate_html_jump(htmlfilename, projectname, css_filename, jumpinfos):
    ...
    def getJumpsHtml():
        ...
        htmldirrepl = htmldir.replace('\\', '/') + '/'
        def jumphtml(jumpinfo):
            reportpath, reportname = jumpinfo
            relpath = os.path.abspath(reportpath).replace('\\', '/')
            relpath = relpath.replace(htmldirrepl, '')
